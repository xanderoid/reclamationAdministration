...
class SCPT_Admin {
	...
	function super_cpt_menu_menu() {
		...
		add_management_page( __( 'SuperCPT', 'oldpaper' ), __( 'SuperCPT', 'oldpaper' ), 'manage_options', 'super_cpt', array( $this, 'scpt_site_settings_page' ) );
	}

	function scpt_site_settings_page() {
		global $scpt_plugin;
		if ( !current_user_can( 'manage_options' ) ) wp_die( __( 'You do not have sufficient permissions to access this page.', 'oldpaper' ) );
